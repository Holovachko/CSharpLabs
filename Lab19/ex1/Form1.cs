    ...
    public partial class Form1 : Form
    {
        ...
        private void кстьПарнихЧиселToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i < numericUpDown1.Value; i++)
                if (Convert.ToDouble(dataGridView1[i, 0].Value) < Convert.ToDouble(dataGridView1[i-1, 0].Value))
                {
                    count = Convert.ToDouble(dataGridView1[i - 1, 0].Value);
                }
                else
                {
                    Convert.ToDouble(dataGridView1[i, 0].Value);
                }
